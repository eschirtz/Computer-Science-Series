...
function naiveHillClimbing(state, state_space) {
  ...
  const successors = state_space.getSuccessors(state);
  let s = state;
  // For each successor
...
function simulatedAnnealing(state, state_space, temp) {
  ...
  const successors = state_space.getSuccessors(state, STEP_SIZE_DEFAULT);
  temp = temp < 0 ? 0 : temp;
  // For each successor
  for (let i = 0; i < successors.length; i++) {
    const t = successors[i];
    ...
    if (t.score > s.score) {
      ...
      const loss = Math.abs(s.score - t.score); // 'badness' of state
